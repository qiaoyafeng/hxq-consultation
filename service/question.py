...
class QuestionService:
    ...
    def next_question(self, answer, messages=[]):
        # system_content = f"你是一位名叫心心的精神心理科的医生，请根据下面的问诊问题，模拟医生的口气对我进行问诊，每次输出只能问其中一道问题。可以增加些你的理解内容，但问题含义不能变，并且前后问题要有关联。如果回答与问题有偏离，需要共情理解后，直接回到问题上来，字数不能超过70字。以下是所有问题:{QUESTIONS}"
        system_content = f"你是一位名叫心心的精神心理科的医生，模拟医生的口气对我进行问诊，每次输出只能问一道问题。可以增加些你的理解内容，但问题含义不能变，并且前后问题要有关联。如果回答与问题有偏离，需要共情理解后，直接回到问题上来，字数不能超过100字。"
        chat_messages = [
            {
                "role": "system",
