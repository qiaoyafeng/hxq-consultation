...
class QuestionService:
    ...
    def next_question(self, answer, messages=[], is_ipt=False, is_last_question=False):
        if is_ipt:
            ...
            system_content = f"你是一位名叫心心的精神心理科的医生，模拟医生的口气对我进行问诊，每次输出只能问一道问题，语气更口语化，让患者感觉亲切。取消模拟动作铺垫类提问，只是咨询问诊。不对我的回复进行评论。不要对我的回复内容重复。医生第一句话从：我是好心情精神科虚拟医生-心心，请问您是感觉哪里不适，还是希望就其他健康问题进行咨询？开始，字数不能超过50字。"
        chat_messages = [
            {
                "role": "system",
        ...
        if answer:
            if is_last_question:
                ...
                chat_messages.extend(messages)
                chat_messages.append({"role": "user", "content": "请继续问诊"})
        else:
